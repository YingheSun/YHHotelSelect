...
public class Setting_List extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		params.addBodyParameter("type", getIntent().getStringExtra("type").toString());
		...
		list_shop.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if (list.get(position).getJumpto().equals("UDALL")) {
					startActivity(All_Outstanding_Orders.class);
				} else if (list.get(position).getJumpto().equals("UDR")) {
					 startActivity(UDROrderList.class);
				} else if (list.get(position).getJumpto().equals("UDC")) {
					 startActivity(UDCOrderList.class);
				} else if (list.get(position).getJumpto().equals("UDZ")) {
					 startActivity(UDZOrderList.class, bundle);
				}
			}
		});
	}
